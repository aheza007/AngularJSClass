...
# Start up MongoDB
Start-Process 'C:/Program Files/MongoDB/Server/3.2/bin/mongod' -RedirectStandardOutput './mongo.log' -RedirectStandardError './mongo.err'
...
if(!(Get-Command "C:/Program Files/MongoDB/Server/3.2/bin/mongoimport")) {
...
Foreach-Object {
  ...
  'C:/Program Files/MongoDB/Server/3.2/bin/mongoimport' --db $dbName --collection $collection --type csv --headerline --file $_
}

# Change the featured product field to a true boolean.
@'
db.products.update({"featured":"true"}, {$set: {featured: true}}, {multi:true});
'@ | 'C:/Program Files/MongoDB/Server/3.2/bin/mongo' $dbName
